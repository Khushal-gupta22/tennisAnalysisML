from utils import (read_video, save_video, measure_distance, draw_player_stats, conver_pixel_to_distance_to_meters)
import constants
...
def main():
    ...
    player_detections = player_tracker.chhose_and_filter_player(court_keypoints, player_detections)
    ...
    # detect ball shots 
    ball_shot_frames = ball_tracker.get_bsll_shot_frames(ball_detections)
    ...
    for ball_shot_ind in range(len(ball_shot_frames)-1):
        ...
        distance_covered_by_ball_in_meters = conver_pixel_to_distance_to_meters(distance_covered_by_ball_in_pixels, 
        ...
        distance_covered_by_opponent_meters = conver_pixel_to_distance_to_meters(distance_covered_by_opponent_pixels, 
                                                                           constants.DOUBLE_LINE_WIDTH,
                                                                           mini_court.get_width_of_mini_court()
                                                                           ) 
    ...
    for i, frame in enumerate(output_video_frames):
        cv2.putText(frame, f"Frame: {i}",(10,30),cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0), 2)
